...
for (( index=0; index<=10; index++ )); do
    ...
    start=$(date +%s.%N)
    $PINPOINT_PATH -c -e MCP2 -o $OUTPUT -- .venv/bin/python3.12 roberta.py --no-resume_from_checkpoint
    end=$(date +%s.%N)
...
for (( index=0; index<=10; index++ )); do
    ...
    start=$(date +%s.%N)
    $PINPOINT_PATH -c -e MCP2 -o $OUTPUT -- .venv/bin/python3.12 roberta.py --resume_from_checkpoint
    end=$(date +%s.%N)
