...
Without this line, Slurm can schedule our jobs on nodes incompatible with the containers' pre-compiled Python binary, resulting in an error on start.\webcite{web_scoreslack}
...
Figure \ref{fig:eval_different_schedulers} and specifically Figure \ref{fig:eval_different_deadlines} show the effect allowing a job to be temporally shifted. 
\todo[inline]{This plot sucks.} 
...
\begin{figure}
    ...
    With increased waiting times, the carbon costs tend to go down. Each dot represents the lowest time a specific carbon-emission can be hit, so if two waiting times result in the same emissions, only the lower is shown.}
    \todo[inline]{Needs improvments if concept itself even makes sense, dots are overlapping}
